...
mu_c = np.zeros(p)
mu_c[:q] = 0+np.random.normal(0, 0.1, q)#, 0.1, 0.15
np.random.seed(42)
...
# 生成均值偏移向量 μc（前q个变量为delta，其余为0）
mu_c = np.zeros(p)
mu_c[:q] = 0.5+np.random.normal(0, 0.1, q)  # 1+np.random.rand(q)
np.random.seed(42)
# 生成数据集（前tau个样本正常，后n_samples-tau个样本偏移）
normal_data = np.random.multivariate_normal(
    mean=np.zeros(p),
...
# 生成均值偏移向量 μc（前q个变量为delta，其余为0）
mu_c = np.zeros(p)
mu_c[:q] = 1+np.random.normal(0, 0.1, q)#1+np.random.rand(q)
np.random.seed(42)
# 生成数据集（前tau个样本正常，后n_samples-tau个样本偏移）
normal_data = np.random.multivariate_normal(
    mean=np.zeros(p),
...
# 生成均值偏移向量 μc（前q个变量为delta，其余为0）
mu_c = np.zeros(p)
mu_c[:q] = 1.5+np.random.normal(0, 0.1, q)#1+np.random.rand(q)
np.random.seed(42)
# 生成数据集（前tau个样本正常，后n_samples-tau个样本偏移）
normal_data = np.random.multivariate_normal(
    mean=np.zeros(p),
...
# 生成均值偏移向量 μc（前q个变量为delta，其余为0）
mu_c = np.zeros(p)
mu_c[:q] = 2+np.random.normal(0, 0.1, q)  # 1+np.random.rand(q)
np.random.seed(42)
# 生成数据集（前tau个样本正常，后n_samples-tau个样本偏移）
normal_data = np.random.multivariate_normal(
    mean=np.zeros(p),
